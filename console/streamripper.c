...
print_status(RIP_MANAGER_INFO *rmi) {
	...
	if (printed_fullinfo && rmi->filename[0]) {
		...
		case RM_STATUS_BUFFERING:
			...
			sprintf(
			    status_str, "buffering - %c ", m_buffer_chars[buffering_tick]);

			print_to_console("[%14s] %.50s\r", status_str, rmi->filename);
			break;
...
parse_arguments(STREAM_PREFS *prefs, int argc, char **argv) {
	...
	for (i = 1; i < argc; i++) {
		...
		case 'f':
			...
			fprintf(
			    stderr,
			    "Error: -f dropstring option is obsolete. "
		...
		case 'P':
			...
			fprintf(
			    stderr,
			    "Error: -P prefix option is obsolete. "
...
parse_extended_options(STREAM_PREFS *prefs, char *rule) {
	...
	/* Splitpoint options */
	if ((!strcmp(rule, "xs-none")) || (!strcmp(rule, "xs_none"))) {
		prefs->sp_opt.xs = 0;

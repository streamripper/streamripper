...
vorbis_process(
	...
	while (ogg_stream_packetout(&stream->os, &packet) > 0) {
		if (inf->doneheaders < 3) {
			if (vorbis_synthesis_headerin(&inf->vi, &inf->vc, &packet) < 0) {
				warn(
				    _("Warning: Could not decode vorbis header "
			...
			if (inf->doneheaders == 3) {
				if (ogg_page_granulepos(page) != 0
					...
					warn(
					    _("Warning: Vorbis stream %d does not have headers "
				...
				for (i = 0; i < inf->vc.comments; i++) {
					...
					if (sep == NULL) {
						warn(
						    _("Warning: Comment %d in stream %d is invalidly "
					...
					for (j = 0; j < sep - inf->vc.user_comments[i]; j++) {
						if (inf->vc.user_comments[i][j] < 0x20
							...
							warn(
							    _("Warning: Invalid comment fieldname in "
					...
					while (j < inf->vc.comment_lengths[i]) {
						...
						if ((val[j] & 0x80) == 0)
							...
							if ((val[j] & 0x20) == 0)
								...
								warn(
								    _("Warning: Illegal UTF-8 sequence in "
							...
							warn(
							    _("Warning: Illegal UTF-8 sequence in comment "
					...
					if (!broken) {
						...
						*sep = 0;
						...
						if (!strcmp(inf->vc.user_comments[i], "artist")
						    ...
						    || !strcmp(inf->vc.user_comments[i], "TRACKNUMBER")
						    || !strcmp(
						           inf->vc.user_comments[i], "Tracknumber")) {
							/* GCS FIX: This is a bit funky, maybe I need
							   to get rid of the ogg built-in utf8 decoder */
							gstring_from_string(
							    rmi,
	...
	if (!header) {
		...
		if (gp > 0) {
...
#ifdef _WIN32
				warn(
				    _("Warning: granulepos in stream %d decreases from %I64d "
				      "to %I64d"),
				    stream->num,
				    inf->lastgranulepos,
				    gp);
#else
				warn(
				    _("Warning: granulepos in stream %d decreases from %lld to "
				      "%lld"),
				    stream->num,
				    inf->lastgranulepos,
				    gp);
			...
			warn(_("Negative granulepos on vorbis stream outside of headers. "
	...
	do {
		...
		case 1:
			...
			if (ogg_page_bos(&rmi->ogg_pg)) {
				...
				ogg_packet packet;
				ogg_stream_init(
				    &rmi->stream.os, ogg_page_serialno(&rmi->ogg_pg));
			...
			** ******************************************************/
			if (ogg_page_bos(&rmi->ogg_pg)) {
				...
				memcpy(
				    rmi->ogg_curr_header + rmi->ogg_pg.header_len,
				    rmi->ogg_pg.body,
				    rmi->ogg_pg.body_len);
			} else if (header) {
				...
				ol->m_header_buf_ptr = 0;
				ol->m_header_buf_len = 0;
				rmi->ogg_curr_header = (char *)realloc(
				    rmi->ogg_curr_header,
				    rmi->ogg_curr_header_len + ol->m_page_len);
				memcpy(
				    rmi->ogg_curr_header + rmi->ogg_curr_header_len,
				    rmi->ogg_pg.header,
				    rmi->ogg_pg.header_len);
				memcpy(
				    rmi->ogg_curr_header + rmi->ogg_curr_header_len

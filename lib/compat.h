...
#define FHANDLE int
// #define OpenFile(_filename_)	open(_filename_, O_RDWR | O_CREAT, S_IRUSR |
// S_IWUSR | S_IRGRP | S_IROTH)
// #define CloseFile(_fhandle_) 	close(_fhandle_)
// #define TruncateFile(_filename_)	CloseFile(open(_filename_, O_RDWR | O_CREAT
...
#define BeginThread(_thandle_, callback, arg)                              \
	{                                                                      \
		_thandle_ =                                                        \
		    (THREAD_HANDLE)_beginthread((void *)callback, 0, (void *)arg); \
	}
...
#define HSEM HANDLE
#define SemInit(_s_) \
	{ _s_ = CreateEvent(NULL, TRUE, FALSE, NULL); }
#define SemWait(_s_)                        \
	{                                       \
		WaitForSingleObject(_s_, INFINITE); \
		ResetEvent(_s_);                    \
	}
...
#define BeginThread(_thandle_, callback, arg) \
	pthread_create(&_thandle_, NULL, (void *)callback, (void *)arg)

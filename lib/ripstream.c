...
static void compute_cbuf2_size(
...
is_track_changed(RIP_MANAGER_INFO *rmi) {
	...
	if (!strcmp(old->artist, cur->artist) && !strcmp(old->title, cur->title)
...
ripstream_rip_mp3(RIP_MANAGER_INFO *rmi) {
	...
	if (rmi->ripstream_first_time_through) {
		...
		compute_cbuf2_size(
		    rmi, &rmi->prefs->sp_opt, rmi->bitrate, rmi->getbuffer_size);
		ret = cbuf2_init(
		    &rmi->cbuf2,
		    rmi->http_info.content_type,
		    GET_MAKE_RELAY(rmi->prefs->flags),
	...
	debug_printf(
	    "rmi->current_track.have_track_info = %d\n",
	...
	if (rmi->current_track.have_track_info && is_track_changed(rmi)) {
		...
		debug_printf(
		    "VERIFIED TRACK CHANGE (find_silence = %d)\n", rmi->find_silence);
	...
	if (cbuf2_get_free(&rmi->cbuf2) < rmi->getbuffer_size) {
		...
		extract_size = rmi->getbuffer_size - cbuf2_get_free(&rmi->cbuf2);
		ret = cbuf2_extract(
		    rmi, &rmi->cbuf2, rmi->getbuffer, extract_size, &curr_song);
		if (ret != SR_SUCCESS) {
		...
		if (curr_song < extract_size) {
			...
			ret = rip_manager_put_data(
			    rmi, &rmi->getbuffer[curr_song], curr_song_bytes);
			if (ret != SR_SUCCESS) {
...
find_sep(RIP_MANAGER_INFO *rmi, u_long *pos1, u_long *pos2) {
	...
	debug_printf(
	    "search window (bytes): [%d,%d] within %d\n",
	    rw_start,
	    rw_end,
...
end_track_mp3(RIP_MANAGER_INFO *rmi, u_long pos1, u_long pos2, TRACK_INFO *ti) {
	...
	if (GET_ADD_ID3V1(rmi->prefs->flags)) {
		...
		string_from_gstring(
		    rmi, id3v1.artist, sizeof(id3v1.artist), ti->artist, CODESET_ID3);
		string_from_gstring(
		    rmi,
		    id3v1.songtitle,
		...
		string_from_gstring(
		    rmi, id3v1.year, sizeof(id3v1.year), ti->year, CODESET_ID3);
	...
	debug_printf(
	    "Current track number %d (skipping if less than %d)\n",
	    rmi->track_count,
	    rmi->prefs->dropcount);
...
start_track_mp3(RIP_MANAGER_INFO *rmi, TRACK_INFO *ti) {
	...
	if (GET_ADD_ID3V2(rmi->prefs->flags)) {
		...
		/* Encoded by */
		ret = write_id3v2_frame(
		    rmi, "TENC", m_("Ripped with Streamripper"), id3_charset, &sent);
...
compute_cbuf2_size(
	...
	mic_to_sw_start =
	    -mi_to_mic + offset - ms_to_bytes(xs_search_window_1, bitrate);
	mic_to_sw_end =
	    -mi_to_mic + offset + ms_to_bytes(xs_search_window_2, bitrate);
	...
	mic_to_cb_start =
	    -((-mic_to_cb_start + (meta_interval - 1)) / meta_interval);

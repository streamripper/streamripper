...
static void
copy_rule_result(
    mchar *dest, mchar *query_string, regmatch_t *pmatch, int idx) {
	debug_printf("copy_rule_result: idx=%d\n", idx);
	if (idx > 0 && idx <= MAX_SUBMATCHES) {
		mstrncpy(
		    dest,
		    query_string + pmatch[idx].rm_so,
...
init_metadata_parser(RIP_MANAGER_INFO *rmi, char *rules_file) {
	...
	while (1) {
		...
		if (ri + 1 != rn) {
			rmi->parse_rules =
			    realloc(rmi->parse_rules, (ri + 1) * sizeof(Parse_Rule));
		...
		if (rmi->parse_rules[ri].cmd == PARSERULE_CMD_SUBST) {
			...
			if (!rbp) {
				printf(
				    "Warning: malformed command in rules file:\n%s\n",
				    rule_buf);
				continue;
			}
		}
		...
		gstring_from_string(
		    rmi, w_match_buf, MAX_RULE_SIZE, match_buf, CODESET_UTF8);
		...
		if (rmi->parse_rules[ri].cmd == PARSERULE_CMD_SUBST) {
			...
			gstring_from_string(
			    rmi, w_subst_buf, MAX_RULE_SIZE, subst_buf, CODESET_UTF8);
...
parse_metadata(RIP_MANAGER_INFO *rmi, TRACK_INFO *ti) {
	...
	gstring_from_string(
	    rmi, query_string, MAX_TRACK_LEN, ti->raw_metadata, CODESET_METADATA);
	for (rulep = rmi->parse_rules; rulep->cmd; rulep++) {
#if !defined(USE_GLIB_REGEX)
		...
		if (rulep->cmd == PARSERULE_CMD_MATCH) {
			debug_mprintf(
			    m_("Testing match rule: ") m_S m_(" vs. ") m_S m_("\n"),
			    query_string,
			    rulep->match);
			if (rulep->flags & PARSERULE_SKIP) {
				...
				copy_rule_result(
				    ti->artist, query_string, pmatch, rulep->artist_idx);
				copy_rule_result(
				    ti->title, query_string, pmatch, rulep->title_idx);
				copy_rule_result(
				    ti->album, query_string, pmatch, rulep->album_idx);
				copy_rule_result(
				    ti->track_p, query_string, pmatch, rulep->trackno_idx);
				copy_rule_result(
				    ti->year, query_string, pmatch, rulep->year_idx);
#endif
			...
			debug_mprintf(
			    m_("Testing subst rule: ") m_S m_(" vs. ") m_S m_("\n"),
			    query_string,
			    rulep->match);
			rc = g_regex_match(rulep->reg, query_string, 0, &match_info);
			if (rc == 0) {
			...
			tmp[start_pos] = 0;
			subst_string = g_strconcat(tmp, rulep->subst, &query_string[end_pos], NULL);
			g_free(tmp);
			g_match_info_free(match_info);

...
int
utf8_mbtowc(int *pwc, const char *s, size_t n) {
	unsigned char c;
	int wc, i, k;
...
int
charset_mbtowc(struct charset *charset, int *pwc, const char *s, size_t n) {
	return (*charset->mbtowc)(charset->map, pwc, s, n);
}

int
charset_wctomb(struct charset *charset, char *s, int wc) {
	return (*charset->wctomb)(charset->map, s, wc);
}

int
charset_max(struct charset *charset) {
	return charset->max;
}
...
static int
mbtowc_utf8(void *map, int *pwc, const char *s, size_t n) {
	return utf8_mbtowc(pwc, s, n);
}

static int
wctomb_utf8(void *map, char *s, int wc) {
...
static int
mbtowc_ascii(void *map, int *pwc, const char *s, size_t n) {
	int wc;
...
static int
wctomb_ascii(void *map, char *s, int wc) {
	if (!s)
		return 0;
	if (wc & ~0x7f)
		return -1;
...
static int
mbtowc_iso1(void *map, int *pwc, const char *s, size_t n) {
	int wc;
...
static int
wctomb_iso1(void *map, char *s, int wc) {
	if (!s)
		return 0;
	if (wc & ~0xff)
		return -1;
...
static int
mbtowc_8bit(void *map1, int *pwc, const char *s, size_t n) {
	struct map *map = map1;
	unsigned short wc;
...
static struct inverse_map *
make_inverse_map(const unsigned short *from) {
	struct inverse_map *to;
	char used[256];
	int i, j, k;
...
int
wctomb_8bit(void *map1, char *s, int wc1) {
	struct map *map = map1;
...
struct charset charset_utf8 = {6, &mbtowc_utf8, &wctomb_utf8, 0};

struct charset charset_iso1 = {1, &mbtowc_iso1, &wctomb_iso1, 0};

struct charset charset_ascii = {1, &mbtowc_ascii, &wctomb_ascii, 0};

struct charset *
charset_find(const char *code) {
	int i;
	...
	for (i = 0; maps[i].name; i++)
		if (!ascii_strcasecmp(code, maps[i].name)) {
			if (!maps[i].charset) {
				maps[i].charset =
				    (struct charset *)malloc(sizeof(struct charset));
				if (maps[i].charset) {
					...
					if (!map) {
						...
						map->from = maps[i].map;
						map->to =
						    0; /* inverse mapping is created when required */
					}
				}
			}

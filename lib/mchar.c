...
//    int vswprintf (wchar_t * ws, size_t n, const wchar_t * format, va_list
//    arg);
#endif
...
subnstr_until(const char *str, char *until, char *newstr, int maxlen) {
	...
	for (len = 0; strncmp(p, until, strlen(until)) != 0 && len < maxlen - 1;
	     p++) {
		newstr[len] = *p;
		len++;
	}
...
char *
format_byte_size(char *str, long size) {
...
convert_string_with_replacement(
	...
	*output_bytes = 0;
	...
	repl_string =
	    g_convert(repl, strlen(repl), to_codeset, "UTF-8", &br, &bw, &error);
	...
	while (cur < input_bytes) {
		...
		br = 0;
		os = g_convert_with_iconv(
		    &input_string[cur], bytes_to_convert, giconv, &br, &bw, &error);
		debug_printf(
		    "cur=%d, btc=%d, br=%d, bw=%d (b1=0x%02x)\n",
		    cur,
		    bytes_to_convert,
		    br,
		    bw,
		...
		*/
		if (error) {
			...
			case G_CONVERT_ERROR_PARTIAL_INPUT:
				debug_printf(
				    "g_convert_with_iconv returned error: %d (%s)\n",
				    error->code,
				    error->message);
				break;
	...
	debug_printf(
	    "convert_string_with_replacement |%s| -> |%s| (%s -> %s)\n",
	    input_string,
	    *output_string,
	    from_codeset,
	    to_codeset);
...
void
lstring_from_lstring(
    Lstring *lstring_out, /* Output: Output string */
...
int
gstring_from_string(
    RIP_MANAGER_INFO *rmi, mchar *m, int mlen, char *c, int codeset_type) {
...
int
string_from_gstring(
    RIP_MANAGER_INFO *rmi, char *c, int clen, mchar *m, int codeset_type) {
	CODESET_OPTIONS *mchar_cs = &rmi->mchar_cs;
	if (clen <= 0)
		return 0;
	*c = 0;

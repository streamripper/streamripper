	...
	while (1) {
		...
		while (md) {
			...
			if (tag_compare(md, ICY_METADATA_TAG) == 0) {
				for (md += strlen(ICY_METADATA_TAG);
				     md[0] && (isdigit(md[0]) == 0);
				     md++)
					;
...
try_port(RELAYLIB_INFO *rli, u_short port, char *if_name, char *relay_ip) {
...
#ifndef WIN32
	{
		...
		int opt = 1;
		setsockopt(
		    rli->m_listensock, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
	}
#endif

	if (bind(rli->m_listensock, (struct sockaddr *)&local, sizeof(local))
	    == SOCKET_ERROR) {
...
relaylib_start_threads(RIP_MANAGER_INFO *rmi) {
	...
	ret = threadlib_beginthread(
	    &rli->m_hthread_accept, thread_accept, (void *)rmi);
	if (ret != SR_SUCCESS)
		return ret;
	...
	while (rli->m_running) {
		...
		while (rli->m_listensock != SOCKET_ERROR && rli->m_running) {
			...
			if (ret == 1) {
				...
				newsock = accept(
				    rli->m_listensock, (struct sockaddr *)&client, &iAddrSize);
				if (newsock != SOCKET_ERROR) {
					// Got successful accept

					debug_printf(
					    "Relay: Client %d new from %s:%hu\n",
					    newsock,
					...
					if (header_receive(newsock, &icy_metadata) == 0
						...
						client_http_header =
						    client_relay_header_generate(rmi, icy_metadata);
						...
						ret = send(
						    ...
						    0);
						debug_printf(
						    "Relay: Sent response header to client %d (%d)\n",
						    ret,
						    header_len);
					...
					if (!good) {
						closesocket(newsock);
						debug_printf(
						    "Relay: Client %d disconnected (Unable to receive "
						    "HTTP header) or cbuf2.buf is NULL\n",
						    newsock);
...
send_to_relay(RIP_MANAGER_INFO *rmi, RELAY_LIST *ptr) {
	...
	while (!done) {
		...
		debug_printf(
		    "Relay: Sending Client %d to the client\n", ptr->m_left_to_send);
		ret = send(
		    ptr->m_sock, ptr->m_buffer + ptr->m_offset, ptr->m_left_to_send, 0);

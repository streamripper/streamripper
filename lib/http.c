...
static error_code http_get_sc_header(
...
http_sc_connect(
	...
	while (1) {
		...
		debug_printf(
		    "http_sc_connect(): calling socklib_open"
		    " host=%s, port=%d\n",
		    url_info.host,
		    url_info.port);
		ret = socklib_open(
		    sock, url_info.host, url_info.port, if_name, rmi->prefs->timeout);
		if (ret != SR_SUCCESS) {
			return ret;
		}
...
http_parse_url(const char *url, URLINFO *urlinfo) {
	...
	if (strchr(url, ':') != NULL) {
		...
		ret = sscanf(
		    ...
		    "%511[^:]:%hu/%252s",
		    urlinfo->host,
		    (short unsigned int *)&urlinfo->port,
		    urlinfo->path + 1);
		if (urlinfo->port < 1)
			return SR_ERROR_PARSE_FAILURE;
		ret -= 1;
...
http_construct_sc_request(
	...
	if (ui.username[0] && ui.password[0]) {
		char *authbuf =
		    make_auth_header("Authorization", ui.username, ui.password);
	...
	if (proxyurl && proxyui.username[0] && proxyui.password[0]) {
		char *authbuf = make_auth_header(
		    "Proxy-Authorization", proxyui.username, proxyui.password);
		strcat(buffer, authbuf);
		free(authbuf);
	}
#endif
...
http_parse_sc_header(const char *url, char *header, SR_HTTP_HEADER *info) {
	...
	extract_header_value(
	    header, info->http_location, "Location:", sizeof(info->http_location));
	extract_header_value(header, info->server, "Server:", sizeof(info->server));
	rc = extract_header_value(
	    header, info->icy_name, "icy-name:", sizeof(info->icy_name));
	if (rc == 0) {
		/* Icecast 2.0.1 */
		rc = extract_header_value(
		    header, info->icy_name, "ice-name:", sizeof(info->icy_name));
	}
	...
	rc =
	    extract_header_value(header, stempbr, "Content-Type:", sizeof(stempbr));
	if (rc == 0) {
		rc = extract_header_value(
		    header, stempbr, "content-type:", sizeof(stempbr));
	}
	if (rc == 0) {
	...
	// Check for Icecast 1
		...
		rc = extract_header_value(
		    header, stempbr, "x-audiocast-bitrate:", sizeof(stempbr));
		if (rc) {
			info->icy_bitrate = atoi(stempbr);
		}
	}
	...
	// Check for Apache
	    ...
	    ((start = (char *)strstr(header, "Apache")) != NULL)
...
/*
 * Constructs a HTTP response header from the SR_HTTP_HEADER struct, if data is
 * null it is not
 * added to the header
 */
error_code
http_construct_sc_response(
    SR_HTTP_HEADER *info, char *header, int size, int icy_meta_support) {
	char *buf = (char *)malloc(size);

#if defined(commentout)
	char *test_header = "HTTP/1.0 200 OK\r\n"
...
http_get_pls(RIP_MANAGER_INFO *rmi, HSOCKET *sock, SR_HTTP_HEADER *info) {
	...
	for (s = 1; s <= 99; s++) {
		...
		if (!extract_header_value(
		        buf, location_buf, buf1, sizeof(location_buf))) {
			break;
		}
		if (s == 1) {
